...
enum MatchStage : uint8_t {
    STAGE_LOBBY        = 0, // In Lobby waiting for ready
    STAGE_LEVEL_SELECT = 1, // Host selecting level (clients see waiting screen)
...
struct LobbyStatePacket {
    ...
    uint8_t occupied_mask; // bits 0..3 (e.g. 0b0011 for 2 players)
    uint8_t ready_mask;    // bits 0..3 (e.g. 0b0011 for ready state)
...
struct ReadyTogglePacket {
    PacketHeader header;
    uint8_t peer_id;
    uint8_t is_ready; // 1 = ready, 0 = not ready
};

// Host Triggers Game / Stage Transition
struct StageChangePacket {
    PacketHeader header;
    uint8_t host_peer_id;
    uint8_t new_stage; // MatchStage
    uint32_t level_id;
};
