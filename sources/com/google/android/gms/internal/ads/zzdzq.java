...
public final class zzdzq {
    public static int zza(SQLiteDatabase sQLiteDatabase, int i) {
        ...
        Cursor cursorZzh = zzh(sQLiteDatabase, i);
        if (cursorZzh.getCount() > 0) {
            cursorZzh.moveToNext();
            i2 = cursorZzh.getInt(cursorZzh.getColumnIndexOrThrow("value"));
        }
        cursorZzh.close();
        return i2;
    }
    ...
    public static long zzb(SQLiteDatabase sQLiteDatabase, int i) {
        long j;
        Cursor cursorZzh = zzh(sQLiteDatabase, 2);
        if (cursorZzh.getCount() > 0) {
            cursorZzh.moveToNext();
            j = cursorZzh.getLong(cursorZzh.getColumnIndexOrThrow("value"));
        } else {
            j = 0;
        }
        cursorZzh.close();
    ...
    public static ArrayList zzc(SQLiteDatabase sQLiteDatabase) {
        ...
        while (cursorQuery.moveToNext()) {
            try {
                arrayList.add(zzayn.zzi(cursorQuery.getBlob(cursorQuery.getColumnIndexOrThrow("serialized_proto_data"))));
            } catch (zzgqy e) {
