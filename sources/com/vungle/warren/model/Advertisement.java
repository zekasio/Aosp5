...
public class Advertisement implements Comparable<Advertisement> {
    ...
    public Advertisement(JsonObject jsonObject) throws IllegalArgumentException {
        ...
        if (JsonUtil.hasNonNull(asJsonObject, "adType")) {
            ...
            if (asString2.equals("vungle_local")) {
                ...
                this.templateId = "";
                this.templateType = "";
            } else if (asString2.equals("vungle_mraid")) {
                this.adType = 1;
                        ...
                        if (JsonUtil.hasNonNull(asJsonObject, "tpat")) {
                            ...
                            if (i != 0) {
                                ...
                                for (int i2 = 0; i2 < 5; i2++) {
                                    int i3 = i2 * 25;
                                    String str = String.format(Locale.ENGLISH, "checkpoint.%d", Integer.valueOf(i3));
                                    this.checkpoints.add(i2, JsonUtil.hasNonNull(asJsonObject3, str) ? new Checkpoint(asJsonObject3.getAsJsonArray(str), (byte) i3) : null);
                                }
                            ...
                            if (!treeSet.isEmpty()) {
                                for (String str2 : treeSet) {
                                    ...
                                    this.dynamicEventsAndUrls.put(str2, arrayList);
                                }
                            }
                        } else {
    ...
    public boolean isNativeTemplateType() {
        return TapjoyConstants.TJC_PLUGIN_NATIVE.equals(this.templateType);
    }
    ...
    public String[] getTpatUrls(String str) {
        ...
        if (i == 0) {
            if (arrayList == null) {
                VungleLogger.warn("Advertisement#getTpatUrls", str2);
    ...
    public Map<String, String> getMRAIDArgsInMap() {
        ...
        HashMap map = new HashMap(this.templateSettings);
        ...
        if (!this.mraidFiles.isEmpty()) {
            map.putAll(this.mraidFiles);
        }
        if (!this.incentivizedTextSettings.isEmpty()) {
            map.putAll(this.incentivizedTextSettings);
        }
        if (!"true".equalsIgnoreCase((String) map.get(START_MUTED))) {
            map.put(START_MUTED, (getAdConfig().getSettings() & 1) == 0 ? TJAdUnitConstants.String.FALSE : "true");
        }
        return map;
    }
    ...
    public Map<String, String> getDownloadableUrls() {
        ...
        int i = this.adType;
        if (i == 0) {
            map.put(KEY_VIDEO, this.videoUrl);
            if (!TextUtils.isEmpty(this.postrollBundleUrl)) {
                map.put(KEY_POSTROLL, this.postrollBundleUrl);
            }
        } else if (i == 1) {
            if (!isNativeTemplateType()) {
                map.put(KEY_TEMPLATE, this.templateUrl);
            }
            Iterator<Map.Entry<String, Pair<String, String>>> it = this.cacheableAssets.entrySet().iterator();
            while (it.hasNext()) {
                String str = (String) it.next().getValue().first;
                if (isValidUrl(str)) {
                    map.put(URLUtil.guessFileName(str, null, null), str);
                }
            }
        } else {
    ...
    void setWinNotifications(List<String> list) {
        if (list == null) {
